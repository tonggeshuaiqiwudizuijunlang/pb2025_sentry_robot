...
std::unique_ptr<Detector> ArmorDetectorNode::initDetector() {
  ...
  int detect_color_param = declare_parameter("detect_color", 0, color_desc);
  EnemyColor detect_color =
      detect_color_param == static_cast<int>(EnemyColor::BLUE)
          ? EnemyColor::BLUE
          : EnemyColor::RED;
...
ArmorDetectorNode::onSetParameters(std::vector<rclcpp::Parameter> parameters) {
  ...
  for (const auto &param : parameters) {
    if (param.get_name() == "binary_thres") {
      ...
      int color = param.as_int();
      detector_->detect_color =
          color == static_cast<int>(EnemyColor::BLUE) ? EnemyColor::BLUE
                                                       : EnemyColor::RED;
