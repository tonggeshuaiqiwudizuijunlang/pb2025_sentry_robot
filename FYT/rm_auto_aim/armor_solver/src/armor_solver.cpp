...
rm_interfaces::msg::GimbalCmd Solver::solve(const rm_interfaces::msg::Target &target,
  ...
  try {
    ...
    // Publish rpy for debugging
    try {
      if (auto node_ptr = node_.lock()) {
        auto pub = node_ptr->create_publisher<std_msgs::msg::Float64MultiArray>(
          "solver/rpy_debug", rclcpp::QoS(10));
        std_msgs::msg::Float64MultiArray arr;
        arr.data = {rpy_[0], rpy_[1], rpy_[2]};
        pub->publish(arr);
      }
    } catch (const std::exception &e) {
      FYT_WARN("armor_solver", "Failed to publish rpy_debug: %s", e.what());
    }
  ...
  gimbal_cmd.fire_advice = false;  // Will be evaluated after angle_offset compensation
  ...
  switch (state) {
    ...
    case TRACKING_CENTER: {
      ...
      // fire_advice handled after angle_offset compensation
  ...
  // Evaluate fire_advice with compensated angles to avoid offset causing no-fire
  ...
  // Publish debug values: cur_yaw, cur_pitch, target_yaw, target_pitch, distance, yaw_diff, pitch_diff
  try {
    if (auto node_ptr = node_.lock()) {
      auto pub = node_ptr->create_publisher<std_msgs::msg::Float64MultiArray>(
        "solver/gimbal_target_debug", rclcpp::QoS(10));
      std_msgs::msg::Float64MultiArray arr;
      double cur_yaw = rpy_[2];
      double cur_pitch = rpy_[1];
      double target_yaw_val = yaw;   // yaw computed for chosen target
      double target_pitch_val = pitch; // pitch computed for chosen target
      double yaw_dev = cur_yaw - target_yaw_val;
      double pitch_dev = cur_pitch - target_pitch_val;
      arr.data = {cur_yaw, cur_pitch, target_yaw_val, target_pitch_val, distance, yaw_dev, pitch_dev};
      pub->publish(arr);
    }
  } catch (const std::exception &e) {
    FYT_WARN("armor_solver", "Failed to publish gimbal_target_debug: %s", e.what());
  }
...
bool Solver::isOnTarget(const double cur_yaw,
  ...
  // too large
  shooting_range_yaw = std::max(shooting_range_yaw, 0.8 * M_PI / 180);  //1.0
  shooting_range_pitch = std::max(shooting_range_pitch, 0.8 * M_PI / 180);  //1.0
