  ...
  const sensor_msgs::msg::PointCloud2::ConstSharedPtr & pcd_msg)
{
  ...
  // Use current time instead of pcd timestamp to avoid TF cache issues
  auto current_time = this->now();
  auto lookup_time = (pcd_msg->header.stamp.nanosec == 0 && pcd_msg->header.stamp.sec == 0)
                       ? current_time
                       : rclcpp::Time(pcd_msg->header.stamp);
  ...
  tf_lidar_to_chassis = getTransform(lidar_frame_, base_frame_, lookup_time);

  tf_odom_to_chassis = tf_odom_to_lidar * tf_lidar_to_chassis;
  tf_odom_to_robot_base = tf_odom_to_lidar * tf_lidar_to_robot_base_;

  publishTransform(
    tf_odom_to_chassis, odometry_msg->header.frame_id, base_frame_, lookup_time);
  publishOdometry(
    tf_odom_to_robot_base, odometry_msg->header.frame_id, robot_base_frame_, lookup_time);

  sensor_msgs::msg::PointCloud2 out;
  pcl_ros::transformPointCloud(lidar_frame_, tf_odom_to_lidar.inverse(), *pcd_msg, out);
  out.header.stamp = lookup_time;  // Update timestamp to lookup time
...
{
  try {
    // Check if transform is available, if not, wait a bit or use latest
    if (!tf_buffer_->canTransform(target_frame, source_frame, time, rclcpp::Duration::from_seconds(0.1))) {
    ...
    RCLCPP_WARN_THROTTLE(
      this->get_logger(), *this->get_clock(), 1000, "TF lookup failed: %s. Returning identity.",
      ex.what());
    return tf2::Transform::getIdentity();
  }
}
